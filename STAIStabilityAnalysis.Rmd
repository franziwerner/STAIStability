...
items.core.tb <- as_tibble(items.core)
...
## alternative way to exclude NAs
#test <- tb.data %>%
#  drop_na(STAIT_Z02, STAIT_T3, STAIT_T4, STAIT_T5)
...
```{r, ICC}
# ICC Calculation with psych (detach("package:DescTools", unload=TRUE))
icc <- c()
pVal.T1 <- c()
lowBound <- c()
upperBound <- c()
...
vbssp.Plt.T1 <- 
  ...
  labs(title = "1st sample: Score for first and second measurement ", x = "Time") +
  ylim(20, 60) +
  theme_apa() +
...
vbssp.Plt.T2 <- ggplot(tbl.T2.long, aes(x = as.numeric(Time), y = Score), fill = Time) + 
  ...
  labs(title = "2nd sample: Score for first and fourth measurement ", x = "Time") +
  ylim(20, 60) +
  theme_apa() +
...
```{r, fig.width = 12, fig.height = 5, fig.cap = "Achieved scores. Shown are the STAI-T scores for respective measurements of the first (A) and second (B) trial and individual changes in score between these measurements. Single points represent the number of participants and the bold blue line indicates the mean change.", warning=FALSE}
...
```{r, mixed model, results='hide'}

#resMM1 <- lmer(score ~ timepoint * item + (1 | id), data = tb.ANOVA.ID)
#summary(resMM1)
...
### wiederholte Messungen und Longitudinaldaten
#res1 <- lmer(score ~ timepoint + item + (1 | id), data = tb.ANOVA.ID, REML = FALSE, na.action = na.omit)
#res2 <- lmer(score ~ timepoint + item + (1 + timepoint | id), data = tb.ANOVA.ID, REML = FALSE, na.action = na.omit)
#anova(res1, res2)
